...
const Button: FunctionComponent<ButtonProps> = ({ onClick, disabled, className, children, icon }) => {
    return (
        <button
            onClick={onClick}
            disabled={disabled}
            className={`px-3 py-2 bg-gray-100 rounded-lg flex ${!icon ? 'justify-center' : 'items-center gap-2'} hover:opacity-75 cursor-pointer ${className} focus:outline-none focus:ring focus:ring-blue-200`}
        >
            {icon ? <>
